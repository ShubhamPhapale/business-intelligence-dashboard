...
def calculate_insights(voter_df, correct_answers_df, n):
    ...
    if 'response_time' in voter_df.columns:
        insights['fast_responses'] = voter_df.groupby('question_text')['response_time'].min().nsmallest(n)
    
    # 10. Slow Responded Questions
    if 'response_time' in voter_df.columns:
        insights['slow_responses'] = voter_df.groupby('question_text')['response_time'].max().nlargest(n)
    ...
    with tab6:
        ...
        if 'response_time' in voter_df.columns:
            fig_response_dist = px.histogram(
                voter_df['response_time'].dt.total_seconds(),
